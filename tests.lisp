  ...
  (is
   (loop
      ...
      do (format t "~&~d ~d ~d ~a"
                 ...
                 (if (equal test-hotp expected-hotp) "ok" "ko"))
      always (equal test-hotp expected-hotp))))
...
(test totp-working
  ...
  (loop
     ...
     as ok? = (string-equal expected-totp test-totp)
     when *verbose* do (format t "~&~A ~12D ~D/~D ~A/~A ~A"
                               (if ok? ". " "KO")
                               time
